 ...
 <!--           
            ...
            <?}?>    
             <div class="cant_x_pagina" style="width: 100%;">Mostrar:&nbsp
                <? $link="index.php?articulo=".$articulo."&agru_1=".$agru_1."&agru_2=".$agru_2."&agru_3=".$agru_3."&modo_grilla=N";?>
            ...
            <h4>
                <a href="articulo.php?cod_articulo=<?=rawurlencode($row['cod_articulo'])?>">
                    <?echo $row['descrip_arti']?>
                </a>
            </h4>
            <h5>Código: <?echo $row['cod_articulo']?></h5>
            <p><?echo $row['desc_adicional']?></p>
            <p style="font-weight: bold;font-size: 18px;"><?echo $row['mone'].' '.cdouble($row['precio_vta'])?> <span style="font-weight: normal;font-size: 12px;">(IVA incl.)</span></p>
...
<!--             <article class="articulos">
    ...
    <p style="font-size: 13px;font-weight: bold;text-align: right;width: 100%;">
        <?  $sql="select mone_coti from monedas where codi_afip='DOL'";
            $result_mone = sqlsrv_query($conn,$sql);
            $row_mone = sqlsrv_fetch_array($result_mone);
            ?>
        Cotización del Dolar: $<?=cdouble($row_mone['mone_coti'])?>
    </p>
